...
public class Server {
        ...
        public static void main(String[] args) throws IOException {
                ...
                BigInteger q = new BigInteger("101103163367");
                BigInteger N = p.multiply(q);
                BigInteger e = new BigInteger("8383132602661586723");
                BigInteger k1 = new BigInteger("15410448351024156590"); // 156
                BigInteger k2 = new BigInteger("11860563476615815838"); // 210
                BigInteger k3 = new BigInteger("17881656493317337359"); // 175

                // System.out.println("client connected");
                // send
                PrintWriter pr = new PrintWriter(s.getOutputStream());
                pr.println(e);
                BigInteger c1 = k1.modPow(e, N);
                BigInteger c2 = k2.modPow(e, N);
                BigInteger c3 = k3.modPow(e, N);
                pr.println(c1);
                pr.println(c2);
                pr.println(c3);
                System.out.println(c1 + " " + c2 + " " + c3);

                BigInteger[] arr = { k1, k2, k3 };

                String[] Array = new String[3];
                for (int i = 0; i < arr.length; i++) {

                        System.out.println("decrypted keys = " + arr[i]);
                        String hexNewValue = arr[i].toString(16);
                        System.out.println("decrypted keys = " + hexNewValue);
                        Array[i] = hexNewValue;

                        for (String y : Array) {
                                System.out.println("hex" + y);
                        }
                }

                String message = "D5DCE920904C8FAE";
                Server cipher = new Server();
                message = cipher.encrypt(message, Array[0]);
                message = cipher.decrypt(message, Array[1]);
                message = cipher.encrypt(message, Array[2]);

                System.out.println("Enc = " + message.toUpperCase());
